...
fn test_completed_order() {
    ...
    let observable = observable.do_before_termination(move |termination| {
        assert_eq!(checker_1_cloned.state(), State::Active);
...
fn test_multiple_operation() {
    ...
    let observable = observable
        .do_before_termination(move |termination| {
            terminations_cloned_1.with_mut(|values| values.push(termination.clone()));
        })
        .do_before_termination(move |termination| {
            terminations_cloned_2.with_mut(|values| values.push(termination.clone()));
        });
