...
fn test_multiple_operation() {
    ...
    assert!(subject_3.on_next(-1).is_continue());
    assert_eq!(checker.values(), [111, 222]);
    assert_eq!(checker.state(), State::Active);
...
fn test_race_condition_next_after_unsub() {
    let mut sender_1 = None;
    let mut sender_2 = None;
    let mut sender = None;
    ...
    assert!(sender.as_mut().unwrap().on_next(observable_1).is_continue());
    assert!(checker.values().is_empty());
    assert_eq!(checker.state(), State::Active);

    assert!(sender.as_mut().unwrap().on_next(observable_2).is_continue());
    assert!(checker.values().is_empty());
    assert_eq!(checker.state(), State::Active);
    ...
    assert!(sender_2.as_mut().unwrap().on_next(111).is_continue());
    assert_eq!(checker.values(), [111]);
    assert_eq!(checker.state(), State::Active);

    assert!(sender_1.as_mut().unwrap().on_next(-1).is_continue());
    assert_eq!(checker.values(), [111]);
    assert_eq!(checker.state(), State::Active);

    sender_2.unwrap().on_termination(Termination::Completed);
    assert_eq!(checker.values(), [111]);
    assert_eq!(checker.state(), State::Completed);
}
...
fn test_race_condition_terminate_after_unsub() {
    ...
    sender_1.unwrap().on_termination(Termination::Completed);
    assert!(checker.values().is_empty());
    assert_eq!(checker.state(), State::Active);
