...
use crate::tests_utils::{DURATION_1_MS, DURATION_5_MS};
...
fn test_completed_without_next() {
    block_on(|runtime| async move {
        ...
        let stream = observable.into_stream();
        let (checker, _subscription) = Checker::from_stream(stream, runtime.clone());
        runtime.sleep(DURATION_5_MS).await; // make sure the stream is ready.
        assert!(checker.values().is_empty());
        assert_eq!(checker.state(), State::Active);
...
fn test_ref() {
    block_on(|runtime| async move {
        ...
        futures::select!(
            _ = stream.next().fuse() => {},
            _ = runtime.sleep(DURATION_5_MS).fuse()=>{}
        );
...
fn test_immediate_next() {
    block_on(|runtime| async move {
        ...
        let (checker, _subscription) = Checker::from_stream(stream, runtime.clone());
        runtime.sleep(DURATION_1_MS).await;
        assert_eq!(checker.values(), [111]);
        assert_eq!(checker.state(), State::Active);
