...
use rx_rust::{
    ...
    operators::{combining::merge::Merge, creating::create::Create},
...
fn test_completed() {
    ...
    // Custom operations
    let observable = observable.merge_with(observable_1);
    ...
    assert!(sender_1.on_next(555).is_continue());
...
fn test_completed_source_and_another_source_are_same() {
    ...
    // Custom operations
    let observable = subject.clone().merge_with(subject.clone());
...
fn test_unsubscribe() {
    ...
    let observable = observable.merge_with(subject_1.clone());
    ...
    assert!(subject_1.on_next(555).is_continue());
    assert_eq!(checker_1.values(), [111, 222, 333]);
    assert_eq!(checker_1.state(), State::Dropped);
    assert_eq!(checker_2.values(), [111, 222, 333, 444, 555]);
    assert_eq!(checker_2.state(), State::Active);
...
fn test_mut_ref() {
    ...
    assert!(sender_1.on_next(&mut value_3).is_continue());
...
fn test_subscribe_by_different_observer() {
    ...
    // Custom operations
    let observable = subject.clone();
    let observable = observable.merge_with(another_source_subject.clone());
    let observable_1 = observable;
    let observable_2 = observable_1.clone();
...
fn test_unsub_on_next_by_take() {
    ...
    // Custom operations
    let observable = observable.merge_with(observable_1).take(1);
...
fn test_multiple_operation() {
    ...
    let observable = observable
        .merge_with(observable_1)
        .merge_with(observable_2)
        .merge_with(observable_3);
    ...
    assert!(sender_3.on_next(555).is_continue());
    assert_eq!(checker.values(), [111, 222, 333, 444, 555]);
    assert_eq!(checker.state(), State::Active);
    assert_eq!(channel_checker.state(), ChannelState::Subscribed);
    assert_eq!(channel_checker_1.state(), ChannelState::Subscribed);
...
fn test_multiple_operation_same_another_source() {
    ...
    // Custom operations
    let observable = observable
        .merge_with(another_source_subject.clone())
        .merge_with(another_source_subject.clone());
    ...
    assert_eq!(checker.values(), [111, 111, 222, 222, 333]);
    assert_eq!(checker.state(), State::Active);
    assert_eq!(channel_checker.state(), ChannelState::Subscribed);
...
fn test_without_convenient_api() {
    ...
    // Custom operations
    let observable = Merge::new(observable, observable_1);
...
fn test_next_on_sub() {
    ...
    let mut subject_1 = BehaviorSubject::new(222);
    let (checker, observer) = Checker::new();

    // Custom operations
    let observable = subject.clone().merge_with(subject_1.clone());
...
fn test_complete_on_sub() {
    ...
    // Custom operations
    let observable = Empty.merge_with(Empty);
...
fn test_error_on_sub() {
    ...
    // Custom operations
    let observable = Throw::new("error").merge_with(observable_1);
...
fn test_next_on_unsub() {
    let (_, observable_1, channel_checker_1) = test_channel::<i32, Infallible>();
    let (checker, observer) = Checker::new();
...
fn test_error_on_unsub() {
    let (_, observable_1, channel_checker_1) = test_channel::<i32, &str>();
    let (checker, observer) = Checker::new();
...
fn test_lifetime_sub() {
...

    {
        ...
        let observable = observable.merge_with(another_source_subject);
...
fn test_lifetime_or() {
...

    {
        ...
        assert!(
            observer
                .on_next((&life_marker_3, &life_marker_3))
                .is_continue()
        );
        let _subscription = observable.subscribe(observer);
    }
...
fn test_lifetime_or_sub() {
...

    {
        ...
        let boundary = Create::new(|observer| {
        ...
        let observable = observable.merge_with(boundary);
...
fn test_type_inference_with_subscribe() {
    ...
    let observable = subject.merge_with(another_source_subject);
