...
fn test_multiple_operation() {
    block_on(|runtime| async move {
        ...
        let observable = observable
            .debounce(DURATION_100_MS, runtime.clone())
            .debounce(DURATION_100_MS + DURATION_20_MS * 2, runtime.clone());
        ...
        runtime.sleep(DURATION_100_MS * 2 + DURATION_20_MS).await;
        assert!(checker.values().is_empty());
        assert_eq!(checker.state(), State::Active);
        assert_eq!(channel_checker.state(), ChannelState::Subscribed);
        ...
        sender.on_next(222);
        runtime.sleep(DURATION_100_MS + DURATION_20_MS * 3).await;
        assert_eq!(checker.values(), [111]);
        assert_eq!(checker.state(), State::Active);
        assert_eq!(channel_checker.state(), ChannelState::Subscribed);
        ...
        sender.on_next(333);
        runtime
            .sleep(DURATION_100_MS * 2 + DURATION_20_MS * 3)
            .await;
