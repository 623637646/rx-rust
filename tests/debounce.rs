...
fn test_completed() {
    block_on(|runtime| async move {
        ...
        // Custom operations
        let observable = observable.debounce(DURATION_LOGICAL, runtime.clone());
...
fn test_unsub_on_next_by_take() {
    block_on(|runtime| async move {
        ...
        let observable = observable
            .debounce(DURATION_LOGICAL, runtime.clone())
            .take(1);
...
fn test_multiple_operation() {
    block_on(|runtime| async move {
        ...
        let observable = observable
            .debounce(DURATION_LOGICAL, runtime.clone())
            .debounce(DURATION_LOGICAL + DURATION_DEVIATION * 2, runtime.clone());
        ...
        runtime
            .sleep(DURATION_LOGICAL * 2 + DURATION_DEVIATION)
            .await;
        assert!(checker.values().is_empty());
        assert_eq!(checker.state(), State::Active);
        assert_eq!(channel_checker.state(), ChannelState::Subscribed);
        ...
        sender.on_next(222);
        runtime
            .sleep(DURATION_LOGICAL + DURATION_DEVIATION * 3)
            .await;
        assert_eq!(checker.values(), [111]);
        assert_eq!(checker.state(), State::Active);
        assert_eq!(channel_checker.state(), ChannelState::Subscribed);
        ...
        sender.on_next(333);
        runtime
            .sleep(DURATION_LOGICAL * 2 + DURATION_DEVIATION * 3)
            .await;
...
fn test_without_convenient_api() {
    block_on(|runtime| async move {
        ...
        // Custom operations
        let observable = Debounce::new(observable, DURATION_LOGICAL, runtime.clone());
...
fn test_immediate_next() {
    block_on(|runtime| async move {
        ...
        // Custom operations
        let observable = subject.clone().debounce(DURATION_LOGICAL, runtime.clone());
...
fn test_immediate_completed() {
    block_on(|runtime| async move {
        ...
        // Custom operations
        let observable = Empty.debounce(DURATION_LOGICAL, runtime.clone());
...
fn test_immediate_error() {
    block_on(|runtime| async move {
        ...
        // Custom operations
        let observable = Throw::new("error").debounce(DURATION_LOGICAL, runtime.clone());
...
fn test_type_inference_with_subscribe() {
    block_on(|runtime| async move {
        // Custom operations
        let observable = Never.debounce(DURATION_LOGICAL, runtime.clone());
