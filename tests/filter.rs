...
fn test_ref() {
    ...
    // Custom operations
    let observable = observable.filter(move |value| value == &&value_1);
...
fn test_unsub_on_next_by_take() {
    ...
    // Custom operations
    let observable = observable.filter(|value| value % 2 == 0).take(1);
...
fn test_multiple_operation() {
    ...
    assert_eq!(checker.values(), [0, 6, 12]);
    assert_eq!(checker.state(), State::Active);
    assert_eq!(channel_checker.state(), ChannelState::Subscribed);
...
fn test_without_convenient_api() {
    ...
    // Custom operations
    let observable = Filter::new(observable, |_| true);
...
fn test_fn() {
    ...
    // Custom operations
    let observable = Just::new(1).filter(|_| {
        s.consume_mut();
        true
    });
...
fn test_type_inference_with_subscribe() {
    // Custom operations
    let observable = Just::new(1).filter(|_| true);
