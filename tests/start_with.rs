...
fn test_unsub_on_next_by_take() {
    ...
    assert_eq!(checker.state(), State::Completed);
    // The prepended values end the stream before the source is subscribed to, so it is never
    // reached: the two remaining ones are not delivered either.
...
fn test_stop_on_next() {
    ...
    let _subscription = observable.subscribe(observer);
    // The observer ended its own stream on the second prepended value, so the third one is never
    // delivered and the source is never subscribed to.
