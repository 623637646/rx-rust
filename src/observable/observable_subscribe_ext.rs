...
pub trait ObservableSubscribeExt<T, E> {
    ...
    fn subscribe_on_event<F>(&mut self, on_event: F) -> impl Cancellable + 'static
    where
        F: for<'a> FnMut(Event<&'a T, E>) + 'static;

    /// Subscribes to the observable with the given `on_next` callback.
    fn subscribe_on_next<F>(&mut self, on_next: F) -> impl Cancellable + 'static
    where
        F: for<'a> FnMut(&'a T) + 'static;
}
    ...
    O: Observable<T, E>,
{
    ...
    fn subscribe_on_next<F>(&mut self, mut on_next: F) -> impl Cancellable + 'static
    where
        F: for<'a> FnMut(&'a T) + 'static,
    {
